"""
Third Year Validation — Deepest Out-of-Sample Test
====================================================
Runs ALL production strategies on the OLDEST available 12-month dataset
(Feb 2023 – Feb 2024) that was NOT used during strategy development.

This is a 3rd year of data — TWO periods removed from the dev period.
If strategies still work here, the edge is structural, not curve-fitted.
...
Usage:
    cd backtesting_engine && python3 strategies/third_year_validation.py
"""
...
def load_third_year_data(instrument):
    """Load 3rd year (Feb 2023 – Feb 2024) data for an instrument."""
...
def prepare_instrument(instrument, sm_index, sm_flow, sm_norm, sm_ema):
    ...
    df = load_third_year_data(instrument)
...
# ---------------------------------------------------------------------------
# Monthly ATR(14) computation
# ---------------------------------------------------------------------------

def compute_monthly_atr(df):
    """Compute monthly ATR(14) for the dataset.

    Uses daily high/low/close, then ATR(14) on daily bars,
    reported per calendar month.
    """
    from zoneinfo import ZoneInfo
    _ET = ZoneInfo("America/New_York")

    # Convert to ET and get dates
    ...
    df_copy = df.copy()
    df_copy['et_date'] = et.date

    # Aggregate to daily bars (RTH: 09:30-16:00 ET)
    et_mins = (et.hour * 60 + et.minute).values
    rth_mask = (et_mins >= 570) & (et_mins < 960)  # 09:30 to 16:00
    df_rth = df_copy[rth_mask]

    daily = df_rth.groupby('et_date').agg({
        'High': 'max',
        'Low': 'min',
        'Close': 'last',
    }).dropna()
    ...
    daily_highs = daily['High'].values
    daily_lows = daily['Low'].values
    daily_closes = daily['Close'].values

    n = len(daily_highs)
    tr = np.zeros(n)
    tr[0] = daily_highs[0] - daily_lows[0]
    for i in range(1, n):
        tr[i] = max(
    ...
    # Monthly average ATR
    monthly_atr = daily[daily['ATR14'] > 0].resample('ME')['ATR14'].mean()
    monthly_atr.index = monthly_atr.index.strftime("%Y-%m")
...
def main():
    print("=" * 80)
    print("THIRD YEAR VALIDATION — DEEPEST OUT-OF-SAMPLE TEST")
    print("Data: Feb 2023 – Feb 2024 (TWO periods removed from development)")
    ...
    mes_min = df_mes['Close'].min()
    mes_max = df_mes['Close'].max()
    print(f"\n  PRICE CONTEXT:")
    print(f"    3rd year MNQ:  {mnq_min:,.0f} – {mnq_max:,.0f}")
    print(f"    Prior year MNQ: ~17,120 – 22,216")
    print(f"    Dev period MNQ: ~20,000 – 24,700")
    print(f"    3rd year MES:  {mes_min:,.0f} – {mes_max:,.0f}")
    print(f"    7pt TP as % of MNQ price: {7/mnq_min*100:.3f}% – {7/mnq_max*100:.3f}% (3rd year)")
    print(f"                               {7/17120*100:.3f}% – {7/22216*100:.3f}% (prior)")
    ...
    # ===================================================================
    print(f"\n{'='*80}")
    print("MONTHLY ATR(14) — 3rd Year (Feb 2023 – Feb 2024)")
    print("=" * 80)

    mnq_monthly_atr = compute_monthly_atr(df_mnq)
    mes_monthly_atr = compute_monthly_atr(df_mes)

    print(f"\n  {'Month':<10} {'MNQ ATR(14)':>12} {'MES ATR(14)':>12}")
    print(f"  {'-'*10} {'-'*12} {'-'*12}")
    all_months = sorted(set(mnq_monthly_atr.index) | set(mes_monthly_atr.index))
    for month in all_months:
        mnq_val = f"{mnq_monthly_atr.get(month, 0):.1f}" if month in mnq_monthly_atr.index else "N/A"
        mes_val = f"{mes_monthly_atr.get(month, 0):.1f}" if month in mes_monthly_atr.index else "N/A"
        print(f"  {month:<10} {mnq_val:>12} {mes_val:>12}")

    if len(mnq_monthly_atr) > 0:
        print(f"  {'-'*10} {'-'*12} {'-'*12}")
        print(f"  {'AVERAGE':<10} {mnq_monthly_atr.mean():>12.1f} {mes_monthly_atr.mean():>12.1f}")

    # ===================================================================
    # COMPARISON TABLE — ALL 3 PERIODS
    # ===================================================================
    print(f"\n{'='*80}")
    print("SIDE-BY-SIDE: 3rd Year vs Prior Year vs Dev Period")
    print("=" * 80)

    # Dev period baselines from MEMORY.md / portfolio_backtest_mar14.md
    # Note: dev period results include gates; validation periods have no gates
    dev_results = {
    ...
    # Prior year results (from prior_year_validation.py runs)
    # These are populated from the known prior year validation results
    prior_results_baseline = {
        "vScalpA":  {"count": 365, "wr": 83.3, "pf": 1.289, "sharpe": 1.917, "net": 1538, "maxdd": -602},
        "vScalpB":  {"count": 479, "wr": 69.9, "pf": 1.157, "sharpe": 1.116, "net": 916,  "maxdd": -502},
        "vScalpC":  {"count": 365, "wr": 77.3, "pf": 1.198, "sharpe": 1.197, "net": 2048, "maxdd": -1080},
        "MES v2":   {"count": 343, "wr": 54.5, "pf": 1.067, "sharpe": 0.572, "net": 1048, "maxdd": -1488},
        "RSI TL":   {"count": 2163, "wr": 70.0, "pf": 1.062, "sharpe": 0.286, "net": 2919, "maxdd": -4058},
    }

    # Build 3rd year results dict
    third_results = {}
    for name, sc in [("vScalpA", sc_a), ("vScalpB", sc_b), ("MES v2", sc_m)]:
        if sc:
            ...
            third_results[name] = None

    if sc_c:
        third_results["vScalpC"] = {
            "count": sc_c["count"], "wr": sc_c["win_rate"], "pf": sc_c["pf"],
            "sharpe": sc_c["sharpe"], "net": sc_c["net_dollar"],
            "maxdd": sc_c["max_dd_dollar"],
        }
    else:
        third_results["vScalpC"] = None

    if sc_tl:
        third_results["RSI TL"] = {
            "count": sc_tl["count"], "wr": sc_tl["win_rate"], "pf": sc_tl["pf"],
            "sharpe": sc_tl["sharpe"], "net": sc_tl["net_dollar"],
            "maxdd": sc_tl["max_dd_dollar"],
        }
    else:
        third_results["RSI TL"] = None

    print(f"\n  {'Strategy':<12} | {'Period':<10} | {'Trades':>6} | {'WR%':>6} | {'PF':>6} | {'Sharpe':>7} | {'Net$':>10} | {'MaxDD$':>9} | Notes")
    print(f"  {'-'*12}-+-{'-'*10}-+-{'-'*6}-+-{'-'*6}-+-{'-'*6}-+-{'-'*7}-+-{'-'*10}-+-{'-'*9}-+-{'-'*20}")

    for name in ["vScalpA", "vScalpB", "vScalpC", "MES v2", "RSI TL"]:
        dev = dev_results[name]
        pri = prior_results_baseline.get(name)
        thi = third_results.get(name)

        # 3rd year row
        if thi:
            thi_dd = f"${thi['maxdd']:,.0f}" if thi['maxdd'] is not None else "N/A"
            print(f"  {name:<12} | {'3RD YEAR':>10} | {thi['count']:>6} | {thi['wr']:>6.1f} | {thi['pf']:>6.3f} | {thi['sharpe']:>7.3f} | ${thi['net']:>+9,.0f} | {thi_dd:>9} | no gates")
        else:
            print(f"  {name:<12} | {'3RD YEAR':>10} | {'NO TRADES':>6} |")

        # Prior year row
        if pri:
            pri_dd = f"${pri['maxdd']:,.0f}" if pri['maxdd'] is not None else "N/A"
            print(f"  {'':12} | {'PRIOR':>10} | {pri['count']:>6} | {pri['wr']:>6.1f} | {pri['pf']:>6.3f} | {pri['sharpe']:>7.3f} | ${pri['net']:>+9,.0f} | {pri_dd:>9} | no gates")
        else:
            print(f"  {'':12} | {'PRIOR':>10} | {'N/A':>6} |")

        # Dev row
        dev_wr = f"{dev['wr']:.1f}" if dev['wr'] is not None else "N/A"
        dev_dd = f"${dev['maxdd']:,.0f}" if dev['maxdd'] is not None else "N/A"
        print(f"  {'':12} | {'DEV':>10} | {dev['count']:>6} | {dev_wr:>6} | {dev['pf']:>6.3f} | {dev['sharpe']:>7.3f} | ${dev['net']:>+9,.0f} | {dev_dd:>9} | {dev['gates']}")

        # Verdict
        if thi and thi['pf'] > 1.0 and thi['net'] > 0:
            pf_ratio = thi['pf'] / dev['pf'] if dev['pf'] > 0 else 0
            if pf_ratio >= 0.9:
                verdict = "STRONG"
            elif pf_ratio >= 0.7:
                verdict = "HOLDS"
                ...
                verdict = "WEAK"
            print(f"  {'':12} | {'VERDICT':>10} | PF ratio vs DEV: {pf_ratio:.1%} — {verdict}")
        elif thi and thi['net'] > 0:
            print(f"  {'':12} | {'VERDICT':>10} | Profitable but PF < 1.0")
        elif thi:
            print(f"  {'':12} | {'VERDICT':>10} | NEGATIVE — strategy FAILS on 3rd year data")
        print()
    ...
    # ===================================================================
    print(f"\n{'='*80}")
    print("FINAL VERDICT — Third Year Validation")
    print("=" * 80)
    ...
    if port4_total > 0:
        print(f"\n  THE EDGE IS STRUCTURAL: Portfolio profitable THREE years back.")
    else:
        print(f"\n  WARNING: Portfolio negative on 3rd year data. Edge may be regime-specific.")

    print(f"\n{'='*80}")
    print("DONE — Third Year Validation Complete")
    print("=" * 80)
